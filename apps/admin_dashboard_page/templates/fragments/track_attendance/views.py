...
def map_js_status_to_db(is_present):
    ...
    return 'ATTENDED' if is_present else 'CANCELLED'
...
def record_attendance(request):
    ...
    try:
        ...
        db_new_status = map_js_status_to_db(is_present)
        js_new_status = map_db_status_to_js(db_new_status)
        ...
        elif db_new_status == 'CANCELLED':  # Marked Absent
            record.attended_at = None
            record.cancelled_at = timezone.now()
        # Note: 'REGISTERED' (Unmarked) status is implicitly handled if you only toggle between ATTENDED and CANCELLED.

        record.save()

        return JsonResponse({
            'message': 'Attendance updated successfully.',
            'new_status': js_new_status
        })
